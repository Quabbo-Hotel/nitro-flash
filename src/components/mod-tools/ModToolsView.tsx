        ...
        <>
            {isVisible &&
                <NitroCardView uniqueKey="mod-tools" className="nitro-mod-tools no-resize" windowPosition={DraggableWindowPosition.TOP_LEFT} theme="primary-modtool" >
                    <NitroCardHeaderView className='header-mt' hideButtonClose headerText={'Mod Tools'} onCloseClick={event => setIsVisible(false)} />
                    <NitroCardContentView className="text-black" gap={1}>
                        ...
                        <Button className={`btn btn-mod-tools position-relative ${(!selectedUser) ? 'pe-none' : ''}`} gap={1} onClick={() => CreateLinkEvent(`mod-tools/toggle-user-info/${selectedUser.userId}`)}>
                            ...
                            <Text overflow="hidden" variant="black" className={`text-nowrap pe-5 ${!selectedUser ? 'me-5 opacity-25' : 'me-4'}`} style={{ marginLeft: '12px', textIndent:"10px" }}>User: {selectedUser ? selectedUser.username : ''}</Text>
                        </Button>
                        ...
                        <Button className="btn btn-mod-tools position-relative" gap={1} onClick={() => setIsFurniFixToolVisible(prevValue => !prevValue)}>
                            <Base className="icon icon-furnifix position-absolute start-1" />
                            <Text overflow="hidden" variant="black" style={{marginLeft:"-12px"}} className="text-nowrap pe-2 me-2">Furnifix Tool</Text>
                        </Button>
